...
NewChannelDialog::NewChannelDialog(ConfigFrame& frame, ConfigSignal* pPreviousChannel, QWidget *parent) :
    QDialog(parent),
...
NewChannelDialog::NewChannelDialog(ValueType valueType, int multipilier, unsigned int divider, int offset,
                          QString channelName, QString unitName, QString comment,
                          ConfigFrame& frame, ConfigSignal* pPreviousChannel, QWidget *parent) :
    QDialog(parent),
    ui(new Ui::NewChannelDialog),
    frame(frame),
    pPreviousChannel(pPreviousChannel)
{
    ...
    ui->divider_spinBox->setValue(static_cast<int>(divider));
...
unsigned int NewChannelDialog::getDivider(){
...
{
    unsigned int thisDLC = frame.getDLC();
    thisDLC += ui->bit16_checkBox->isChecked() ? 2 : 1;
    if (pPreviousChannel != nullptr){
        thisDLC -= pPreviousChannel->getLengthBits()/8;
    }
