...
static GPSDriver_Status_TypeDef _GPSDriver_getUBXChecksumValue(const uint8_t* pSentence, uint16_t length, uint16_t* retChecksumValueLSB);
...
static GPSDriver_Status_TypeDef _GPSDriver_sendASCIICommandAndWaitForResponse(volatile Ublox8MGPSDriver_TypeDef* pSelf, uint8_t* pCommandBuffer, uint8_t* pExpectedOKResponseBuffer);
static GPSDriver_Status_TypeDef _GPSDriver_sendASCIICommand(volatile Ublox8MGPSDriver_TypeDef* pSelf, uint8_t* pCommandBuffer);
static GPSDriver_Status_TypeDef _GPSDriver_sendBinaryCommand(volatile Ublox8MGPSDriver_TypeDef* pSelf, uint8_t* pCommandBuffer, uint16_t length);
static GPSDriver_Status_TypeDef _GPSDriver_sendTestACKCommand(volatile Ublox8MGPSDriver_TypeDef* pSelf);
...
static GPSDriver_Status_TypeDef _GPSDriver_handleGPGGASentence(volatile Ublox8MGPSDriver_TypeDef* pSelf, _GPSDriver_NMEASentenceString* pNmeaSentenceString);
static GPSDriver_Status_TypeDef _GPSDriver_handleGPGSASentence(volatile Ublox8MGPSDriver_TypeDef* pSelf, _GPSDriver_NMEASentenceString* pNmeaSentenceString);
static GPSDriver_Status_TypeDef _GPSDriver_handleGPRMCSentence(volatile Ublox8MGPSDriver_TypeDef* pSelf, _GPSDriver_NMEASentenceString* pNmeaSentenceString);
...
GPSDriver_Status_TypeDef GPSDriver_init(volatile Ublox8MGPSDriver_TypeDef* pSelf, UartDriver_TypeDef* pUartDriverHandler, UartReceiver_TypeDef* pUartReceiverHandler, MSTimerDriver_TypeDef* pMSTimer, Config_GPSFrequency_TypeDef frequency){
	...
	uint8_t ubxCfgPrtCommand[] = {
			...
			(1<<0), //< bitfield txReady LSB EnableTX Ready
			0x00, //< bitfield txReady MSB

			(0x3 << 6), //< bitfield mode LSB (8bit)
			(0b100 << 1), //< no parity
			0x00,
			0x00,

			GPS_UART_BAUDRATE & 0xFF,
			(GPS_UART_BAUDRATE >> 8) & 0xFF,
			(GPS_UART_BAUDRATE >> 16) & 0xFF,
			(GPS_UART_BAUDRATE >> 24) & 0xFF,

			1<<1, //< in nmea protocol
			0x00,

			1<<1, //< out nmea protocol
			0x00,
	...
	if ((ret = _GPSDriver_getUBXChecksumValue(ubxCfgPrtCommand, 21, (uint16_t*)(ubxCfgPrtCommand+21))) != GPSDriver_Status_OK){
		return ret;
	}

	if ((ret = _GPSDriver_sendBinaryCommand(pSelf, ubxCfgPrtCommand, 23)) != GPSDriver_Status_OK){
		return ret;
	}
	...
	if (ret == GPSDriver_Status_OK){
		ret = _GPSDriver_sendTestACKCommand(pSelf);
	}
...
static GPSDriver_Status_TypeDef _GPSDriver_sendASCIICommandAndWaitForResponse(volatile Ublox8MGPSDriver_TypeDef* pSelf, uint8_t* pCommandBuffer, uint8_t* pExpectedOKResponseBuffer){
	...
	ret = _GPSDriver_sendASCIICommand(pSelf, pCommandBuffer);
...
static GPSDriver_Status_TypeDef _GPSDriver_sendTestACKCommand(volatile Ublox8MGPSDriver_TypeDef* pSelf){
	...
	return _GPSDriver_sendASCIICommandAndWaitForResponse(pSelf, commandBuffer, responseBuffer);
}
...
static GPSDriver_Status_TypeDef _GPSDriver_changeUartBaudrateCommand(volatile Ublox8MGPSDriver_TypeDef* pSelf, uint32_t baudRate){
	...
	if ((ret = _GPSDriver_sendASCIICommand(pSelf, buffer)) != GPSDriver_Status_OK){
		return ret;
	}
	...
	return _GPSDriver_sendTestACKCommand(pSelf);
}
