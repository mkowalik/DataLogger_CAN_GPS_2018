...
CANData_Status_TypeDef CANData_GetValueOfSignal(CANData_TypeDef* pData, ConfigSignal_TypeDef* pSignal, uint64_t* pRetSignalVal){
	...
	if (pData == NULL || pSignal == NULL || pRetSignalVal == NULL){
	...
	*pRetSignalVal		= 0;
	...
	if ((pSignal->valueType & CONFIG_VALUE_TYPE_BIG_ENDIAN_TYPE_flag) != 0){ //< Big Endian
		while (bitsLeft > 0) {
			...
			*pRetSignalVal |= (((uint64_t)actualByte) << (bitsLeft - (((bitsLeft % 8u) == 0) ? 8u : (bitsLeft % 8u))) );
		...
		while (bitsLeft > 0) {
			...
			*pRetSignalVal |= (((uint64_t)actualByte) << (((bitIt - pSignal->startBit)/8) * 8u)); //TODO Tu raczej blad
